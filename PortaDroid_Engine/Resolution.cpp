...
// Purpose        : Iterative impulse collision resolution system.
// History        :
// -9/10/16  : created
//           : Reset()
//           : GetNextContact()
...
{
  /*Resets the number of contacts to 0*/
  void ContactSet::Reset()
  {
    NumberOfContacts = 0;
  }

  /*Returns the next contact in the ContactSet (colliding objects)*/
  BodyContact *ContactSet::GetNextContact()
  {
    if (NumberOfContacts == MaxContacts)
    {
      OutputDebugStringA("Max number of contacts reached. Too many colliding objects.\n");
      //MessageBox.Show("Max number of contacts reached. Too many colliding objects.\n");
    }
  ...
  {
    //get good results by considering all contacts at once at solving them together. This method basically 
    //just keeps poking the bodies until the penetrations are below the penetration slop epsilon. 
    //Better solutions involve building contact graphs, shock propagation and constraint based solvers.
    ...
    while (iterationsRun < maxIterations)
    {
      // Find biggest penetration greater than the correction epsilon
  ...
  void ResloveContactVelocity(BodyContact& c, float dt)
  {
    ...
    if (AccelerationBuildUp)
    {
      ...
      Vec3 accCausedVelocity = c.Bodies[0]->Acceleration - c.Bodies[1]->Acceleration;
    ...
    Vec3 impulsePerIMass = c.ContactNormal * impulse;
